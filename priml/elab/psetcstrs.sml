...
struct
  ...
  fun psconstraints_solver (ctx: PrioSet.set PSEvarMap.map) (constraints: psconstraint list) = 
    let 
      fun check_sup_constraint (s1, s2) = PrioSet.equal (PrioSet.difference (s1, s2), PrioSet.empty)

      fun solver_fold (constraint, ctx) = 
        case constraint of 
          PSCons _ => ctx
    ...
    in 
      ...
      in
        if PSEvarMap.collate PrioSet.compare (ctx', ctx) = EQUAL then ctx'
        else psconstraints_solver ctx' constraints
      end
    end

  (* check (Sync) PSCons constraints in solved system *)
  fun check_psconstraint ctx psctx constraints = 
  let 
