...
struct
    ...
    fun occurs (r : typ ebind ref) x =
        (case x of
           ...
           | Arrow (_, t1, t2) => List.exists (fn (_, t) => occurs r t) t1 
                                  orelse occurs r t2
           | Arrows al => List.exists (fn (_, tl, t) =>
                                       List.exists (fn (_, t) => occurs r t) tl
...
(*           | (At _, _) => raise Unify "tycon mismatch (at)"
	...
	in
        (case (t1, t2) of
           ...
           | (Arrow (_, dom1, cod1), Arrow (_, dom2, cod2)) => 
                 let in
                     ListUtil.all2 (fn ((_, a), (_, b)) => (unifyex ctx eqmap a b;
                                                  true)) 
                                        dom1 dom2
                        orelse raise Unify "Arrow domains have different arity";
...
(*           | (At _, _) => raise Unify "tycon mismatch (at)"
    ...
    fun unify ctx t1 t2  = 
      unifyex ctx (Variable.Map.empty, Variable.Map.empty) t1 t2
