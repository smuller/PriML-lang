...
struct
  ...
  let
    ...
    (* Check / document escaping. *)
    val _ = hasmatch ".+" "aa" orelse raise Nope
    val _ = hasmatch "a.*b" "azzb" orelse raise Nope
    val _ = hasmatch "a.*b" "ab" orelse raise Nope

    val _ = hasmatch "<pre>" "pre" andalso raise Nope
    val _ = hasmatch "<pre>" "<pre>" orelse raise Nope
    (* doesn't work. ^/$ not supported? or wants windows line endings? *)
    (* val _ = hasmatch "^food$" "hello\nfood\nworld" orelse raise Nope *)
  in
    ()
